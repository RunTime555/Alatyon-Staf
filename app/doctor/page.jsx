...
import {
  Beaker, ChevronRight, Loader2, LayoutDashboard,
...
function Sidebar({ open, onClose, collapsed, onLogout }) {
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 bg-[#003a66] flex flex-col transition-all duration-300 ease-in-out
        ...
        <div className="p-3 border-t border-white/10 shrink-0">
          {!collapsed ? (
            <div className="flex items-center gap-3 px-2 py-2">
              ...
              <button onClick={onLogout} title="Log out" className="text-white/30 hover:text-red-400 transition-colors">
                <LogOut size={14} />
              </button>
            </div>
          ) : (
...
export default function DoctorDashboard() {
  ...
  // ✅ FIX: separate stats state — fetched from dedicated API
  ...
  useEffect(() => {
    ...
    // ✅ FIX: fetch real stats from dedicated endpoint
  ...
  // ✅ FIX: stat cards now use real DB counts from /api/doctor/stats
  const statCards = [
    { label: "Pending Reviews", value: stats.pending,       icon: Clock3,       color: "text-blue-600",   bg: "bg-blue-50"   },
    { label: "Reviewed Today",  value: stats.reviewedToday, icon: CheckCircle2, color: "text-emerald-600",bg: "bg-emerald-50"},
    { label: "Critical Cases",  value: stats.critical,      icon: AlertCircle,  color: "text-red-600",    bg: "bg-red-50"    },
    { label: "AI Analyses",     value: stats.aiAnalyses,    icon: TrendingUp,   color: "text-purple-600", bg: "bg-purple-50" },
  ];
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="bg-white border-b border-blue-100 px-4 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="flex-1 relative max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search patients or tests…"
          ...
          <div className="ml-auto shrink-0">
            <button className="relative w-9 h-9 rounded-xl hover:bg-[#f0f6ff] flex items-center justify-center text-slate-500 transition-colors">
              <Bell size={16} />
              {stats.pending > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full" />}
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          {/* Stats grid — now uses real DB counts */}
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-3xl p-12 sm:p-16 text-center border border-dashed border-blue-200">
              ...
              <p className="text-slate-500 font-bold text-sm">{search ? "No results match your search." : "No pending results to review."}</p>
              {search && <button onClick={() => setSearch("")} className="mt-3 text-xs text-blue-500 hover:text-blue-700 font-semibold">Clear search</button>}
            </div>
          ) : (
                  ...
                  <div key={item.id} className="bg-white rounded-2xl border border-blue-50 p-4 sm:p-5 flex items-center gap-3 sm:gap-4 hover:shadow-md hover:border-blue-200 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {item.severity && (
                          <span className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0
                            ${item.severity === "critical" ? "bg-red-50 text-red-600" :
                              item.severity === "elevated" ? "bg-amber-50 text-amber-600" :
                      ...
                      <p className="text-xs text-slate-400 font-semibold truncate">
                        {item.patient?.name ?? "Unknown patient"}<span className="mx-1.5 text-slate-300">·</span>MRN: {item.patient?.mrn ?? "—"}
                      </p>
                    </div>
                    {item.createdAt && (

...
import {
  ...
  ChevronRight, FlaskConical, ChevronDown
} from "lucide-react";
...
export default function DoctorPatients() {
  ...
  const patients = useMemo(() => {
    ...
    results.forEach(r => {
      ...
      if (!map[mrn]) map[mrn] = { mrn, name: r.patient?.name ?? "Unknown", results: [] };
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="bg-white border-b border-blue-100 px-4 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="flex-1 relative max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by patient name or MRN…"
              ...
              {patients.map((patient) => {
                ...
                return (
                  <div key={patient.mrn} className="bg-white rounded-2xl border border-blue-50 overflow-hidden hover:border-blue-200 transition-all">
                    <button onClick={() => setExpanded(isOpen ? null : patient.mrn)} className="w-full flex items-center gap-4 p-4 sm:p-5 text-left">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="bg-blue-50 text-blue-600 text-[10px] font-black px-2.5 py-1 rounded-full">
                          {patient.results.length} test{patient.results.length !== 1 ? "s" : ""}
                        </span>
                      ...
                      <div className="border-t border-blue-50 px-4 sm:px-5 py-3 space-y-2 bg-[#f7faff]">
                        {patient.results.map((r, i) => (
                          <div key={i} className="flex items-center gap-3 bg-white rounded-xl border border-blue-50 p-3">
                            <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center shrink-0"><FlaskConical size={14} className="text-blue-600" /></div>
                            <div className="flex-1 min-w-0">

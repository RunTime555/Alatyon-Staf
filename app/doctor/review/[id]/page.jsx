...
import { Brain, CheckCircle, Loader2, ArrowLeft } from "lucide-react";
...
export default function ReviewPage({ params }) {
  ...
  const runAI = async () => {
    ...
    try {
      ...
      setIsAnalyzing(false);           // ✅ always resets even if fetch throws
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <Button variant="ghost" onClick={() => router.back()} className="mb-6 gap-2 text-slate-500 font-black text-[10px] uppercase">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-[30px] border border-slate-100 shadow-sm h-fit">
            ...
            <p className="text-lg font-black text-slate-800">{data?.patient?.name}</p>      {/* ✅ optional chaining */}
            ...
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-black text-slate-800 uppercase text-xs flex items-center gap-2">
                  <Brain className="text-blue-600" /> AI Diagnostic Summary
                </h3>
